...
public class BedwarsStats {
	...
	public static boolean playerExists(String uuid){
		...
		try {
			...
			ResultSet rs = Main.mysql.querry("SELECT * FROM Bedwars WHERE UUID = '"+ uuid + "'");
	...
	public static void createPlayer(String uuid, String name){
		...
		if(!(playerExists(uuid))){
				name1 = name;
			Main.mysql.update("INSERT INTO Bedwars(UUID, KILLS, TODE, WINS, GAMES, NAME, BETTEN) VALUES ('" +uuid+ "', '0', '0', '0', '0', '"+name1+"', '0');");
		}
	}
	...
	public static Integer getBetten(String uuid, String name){
		Integer i = 0;
		if(playerExists(uuid)){
			try {
				@SuppressWarnings("static-access")
				...
				if((!rs.next()) || (Integer.valueOf(rs.getInt("BETTEN")) == null));
	...
	public static void setKills(String uuid, Integer kills, String name){
		...
		if(playerExists(uuid)){
			Main.mysql.update("UPDATE Bedwars SET KILLS= '" + kills+ "' WHERE UUID= '" + uuid+ "';");
		}else{
			createPlayer(uuid, name);
	...
	public static void setTode(String uuid, Integer tode, String name){
		...
		if(playerExists(uuid)){
			Main.mysql.update("UPDATE Bedwars SET TODE= '" + tode+ "' WHERE UUID= '" + uuid+ "';");
		}else{
			createPlayer(uuid, name);
	...
	public static void setWins(String uuid, Integer tode, String name){
		...
		if(playerExists(uuid)){
			Main.mysql.update("UPDATE Bedwars SET WINS= '" + tode+ "' WHERE UUID= '" + uuid+ "';");
		}else{
			createPlayer(uuid, name);
	...
	public static void setGames(String uuid, Integer tode, String name){
		...
		if(playerExists(uuid)){
			Main.mysql.update("UPDATE Bedwars SET GAMES= '" + tode+ "' WHERE UUID= '" + uuid+ "';");
		}else{
			createPlayer(uuid, name);
	...
	public static void setBetten(String uuid, Integer tode, String name){
		
		if(playerExists(uuid)){
			Main.mysql.update("UPDATE Bedwars SET BETTEN= '" + tode+ "' WHERE UUID= '" + uuid+ "';");
		}else{
			createPlayer(uuid, name);
			setBetten(uuid, tode, name);
		}
	}
	...
	public static void setName(String uuid, String name){
		...
		if(playerExists(uuid)){
			Main.mysql.update("UPDATE Bedwars SET NAME= '" + name+ "' WHERE UUID= '" + uuid+ "';");
		}
	}
	...
	public static void addBetten(String uuid, Integer tode, String name){
		
		if(playerExists(uuid)){
			setBetten(uuid, Integer.valueOf(getBetten(uuid, name).intValue() + tode.intValue()), name);
		}else{
			createPlayer(uuid, name);
			addBetten(uuid, tode, name);
		}
	}
	...
	public static void removeBetten(String uuid, Integer tode, String name){
		
		if(playerExists(uuid)){
			setBetten(uuid, Integer.valueOf(getBetten(uuid, name).intValue() - tode.intValue()), name);
		}else{
			createPlayer(uuid, name);
			removeBetten(uuid, tode, name);
		}
	}
...
//	======================================================================================
	public static Integer getUserRanking(String uuid){
		@SuppressWarnings("static-access")
		ResultSet rs = Main.mysql.querry("SELECT * FROM Bedwars ORDER BY WINS DESC");
		int count = 0;
		try {
			while(rs.next()){
				count++;
